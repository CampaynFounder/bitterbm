            ...
            {(rt.nestedTableChecks ?? []).map((nc, i) => (
              <div key={i} style={{ marginBottom: "var(--space-sm)", padding: "var(--space-sm)", background: "var(--bg-elevated)", borderRadius: 8, border: "1px solid var(--border)" }}>
                ...
                <div style={{ display: "flex", gap: "var(--space-sm)", flexWrap: "wrap", alignItems: "center", marginBottom: "var(--space-xs)" }}>
                  ...
                  <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>Expand selector:</span>
                  <input value={nc.expandSelector ?? ""} onChange={(e) => { const v = e.target.value; const arr = [...(rt.nestedTableChecks ?? [])]; arr[i] = { ...arr[i], expandSelector: v === "" ? undefined : v }; update("resultTable.nestedTableChecks", arr) }} placeholder="e.g. img[src*='add.png']" style={{ ...inputStyle, width: 140 }} title="CSS selector to click to expand hidden sibling row (optional)" />
                  <label style={{ display: "flex", alignItems: "center", gap: 4, fontSize: "0.8125rem" }}><input type="checkbox" checked={nc.collapseAfter ?? false} onChange={(e) => { const arr = [...(rt.nestedTableChecks ?? [])]; arr[i] = { ...arr[i], collapseAfter: e.target.checked }; update("resultTable.nestedTableChecks", arr) }} />Collapse after</label>
                </div>
                <div style={{ display: "flex", gap: "var(--space-sm)", flexWrap: "wrap", alignItems: "center", marginBottom: "var(--space-xs)" }}>
