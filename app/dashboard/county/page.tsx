...
export default function CountyAnalysisPage() {
  ...
  useEffect(() => {
    ...
    supabase.from("user_profiles").select("state, county").eq("id", user.id).maybeSingle().then(({ data }) => {
      const d = data as { state?: string; county?: string } | null
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!county.trim() || !state) return
    setSubmitting(true)
    try {
      await new Promise((r) => setTimeout(r, 1200))
      setResult("County analysis will surface local precedent for alienation cases. Backend integration coming soon.")
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <UpgradeGate plan={plan} feature="County Analysis">
