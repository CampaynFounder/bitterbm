...
export default function GalAnalysisPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setResult("GAL analysis will tailor recommendations to resonate with your Guardian ad Litem. Backend integration coming soon.")
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <UpgradeGate plan={plan} feature="GAL Analysis">
      <main className="section" style={{ minHeight: "80vh", paddingTop: "var(--space-2xl)", paddingBottom: "var(--space-2xl)" }}>
        <div className="container" style={{ maxWidth: 560, margin: "0 auto", animation: "fadeInUp 0.4s var(--ease-out-expo) both" }}>
          ...
          <h1 style={{ fontSize: "1.5rem", marginBottom: "var(--space-sm)", color: "var(--text-primary)", textAlign: "center" }}>GAL Analysis</h1>
          <p style={{ fontSize: "0.9375rem", color: "var(--text-secondary)", marginBottom: "var(--space-xl)", lineHeight: 1.6, textAlign: "center" }}>Tailor your documentation strategy to resonate with your Guardian ad Litem.</p>
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "var(--space-lg)", alignItems: "center" }}>
            <div style={{ width: "100%" }}>
              <label style={{ display: "block", fontSize: "0.875rem", marginBottom: "var(--space-xs)", color: "var(--text-secondary)" }}>GAL name (if known)</label>
              <input type="text" value={galName} onChange={(e) => setGalName(e.target.value)} placeholder="e.g. Jane Smith" style={inputStyle} />
            </div>
            <div style={{ width: "100%" }}>
            ...
            <div style={{ width: "100%", display: "flex", justifyContent: "center" }}>
              <button type="submit" disabled={submitting} className="btn-primary" style={{ width: "100%", maxWidth: 320 }}>{submitting ? "Analyzing…" : "Analyze GAL Approach"}</button>
            </div>
          </form>
