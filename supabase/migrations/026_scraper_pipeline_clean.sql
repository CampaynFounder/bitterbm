...
-- Version: Safe migration with DROP IF EXISTS

-- Enable vector extension if not already enabled
CREATE EXTENSION IF NOT EXISTS vector;

-- Drop existing tables if they exist (in reverse dependency order)
DROP TABLE IF EXISTS scraper_review_queue CASCADE;
DROP TABLE IF EXISTS scraper_queue CASCADE;
DROP TABLE IF EXISTS scraped_doc_chunks CASCADE;
DROP TABLE IF EXISTS scraped_documents CASCADE;
DROP TABLE IF EXISTS scraped_cases CASCADE;
DROP TABLE IF EXISTS scraper_supersets CASCADE;
DROP TABLE IF EXISTS scraper_configs CASCADE;
DROP TABLE IF EXISTS scraper_attorneys CASCADE;
DROP TABLE IF EXISTS scraper_judges CASCADE;
DROP TABLE IF EXISTS scraper_counties CASCADE;
DROP FUNCTION IF EXISTS match_scraped_documents(VECTOR, FLOAT, INT, UUID, TEXT, TEXT);

-- Court Systems/Counties being scraped
CREATE TABLE scraper_counties (
...
CREATE TABLE scraper_configs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  county_id UUID NOT NULL REFERENCES scraper_counties(id) ON DELETE CASCADE,
...
CREATE TABLE scraper_supersets (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  county_id UUID NOT NULL REFERENCES scraper_counties(id) ON DELETE CASCADE,
  name TEXT,
...
CREATE TABLE scraped_documents (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  case_id UUID NOT NULL REFERENCES scraped_cases(id) ON DELETE CASCADE,
...
CREATE TABLE scraped_doc_chunks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  document_id UUID NOT NULL REFERENCES scraped_documents(id) ON DELETE CASCADE,
  case_id UUID NOT NULL REFERENCES scraped_cases(id) ON DELETE CASCADE,
...
-- Judge Analytics
CREATE TABLE scraper_judges (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  county_id UUID NOT NULL REFERENCES scraper_counties(id),
  name TEXT NOT NULL,
  total_cases INTEGER DEFAULT 0,
...
-- Attorney Analytics
CREATE TABLE scraper_attorneys (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL,
...
CREATE INDEX idx_scraped_cases_county ON scraped_cases(county_id);
CREATE INDEX idx_scraped_cases_judge ON scraped_cases(judge);
CREATE INDEX idx_scraped_cases_filed_date ON scraped_cases(filed_date);
CREATE INDEX idx_scraped_documents_case ON scraped_documents(case_id);
CREATE INDEX idx_scraped_doc_chunks_case ON scraped_doc_chunks(case_id);
CREATE INDEX idx_scraper_queue_status ON scraper_queue(status, task_type);
CREATE INDEX idx_scraper_review_status ON scraper_review_queue(status, review_type);
...
-- CREATE POLICY "Allow authenticated access" ON scraper_counties FOR ALL TO authenticated USING (true);
